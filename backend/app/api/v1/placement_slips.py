...
async def parse_upload(
    ...
    # Route each extracted policy period into the product term's start/end
    # dates. Life sheets may additionally auto-fill their FCL / NEL thresholds.
    # All fields fill blanks only, so a broker's manual entry always wins.
    period_autofilled = 0
    nel_autofilled = 0
    for product_slip in parsed.products:
        ...
        if product is None:
            continue
        extracted_window = parse_period_of_insurance(header.period)
        if extracted_window and autofill_term_window(
            db,
            policy_year_id,
            product.id,
            extracted_window[0],
