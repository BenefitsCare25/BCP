...
# Fields worth a vision re-check (amount, date, provider, invoice no.). For
# these, "the claim states a value but no document shows it" (MISSING_IN_PDF)
# is a substantiation gap that must FLAG — not something the aggregate
# confidence score can paper over — once the vision pass has had its chance to
# confirm it (vision_verify flips a confirmed value back to MATCH first).
...
def compute_verdict(
    ...
    for c in field_comparisons:
        ...
        elif status == "MISSING_IN_PDF" and c.get("field_name") in _VISION_FIELDS:
