...
export interface BrokerClaim {
  ...
  /** WHICH SURFACE made that correction — `"member"`, `"hr"` or `"broker"`,
   *  null on a claim amended before the column existed. The queue's Amended
   *  chip reads THIS: `amended_at` is stamped by the assessor's own correction
   *  too, so a chip on the timestamp flagged their own save back at them. */
  amended_by: "member" | "hr" | "broker" | null;
