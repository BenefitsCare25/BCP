...
export function DocumentEditor({ doc, row, editable, busy, save, action, remove }: Props) {
  ...
  return <div className="space-y-4">
    ...
    <p className="text-xs text-muted-foreground">{doc.status === "untagged" ? "Unused uploads may be removed from the incident; stored evidence and audit history are retained." : "Tagged evidence is retained. Upload a correction and link it to this document rather than removing the original."}</p>
